...
**✅ Both phones confirmed on v1.2.0 (2026-07-22)** — Cliff + Cory both show "On the Way (Beta) · v1.2.0" in Settings footer, so ALL of today's mobile fixes (incl. the tracking-number dedupe fix, forward-to-choice) are live on both devices. (Runtime-version note: because app.json bumped 1.1.0→1.2.0, OTA updates published from the 1.2.0 codebase only reach 1.2.0 binaries — both are on 1.2.0, so this is moot now, but keep in mind if publishing OTA for a device on an older binary.)

**▶ NEXT ACTION when resuming: confirm Cory's FedEx package now shows on his profile in the app.** Both phones are on v1.2.0 with the dedupe fix (mobile `76c0a83`/OTA `92966b68` + backend `0b2c4ab`), so the last unverified item is simply: does Cory's FedEx package (tracking `926129...`, sent only to his tracking address) now appear on the Active list with a "Co" chip, under Everyone and Cory? It was hidden before because `deduplicatePackages` collapsed it into Cliff's FedEx card (old carrier+month key → now keyed by tracking number). **If YES → Pillar 1 multi-member flow fully proven with two real users' real packages.** If still missing → next suspect is the email parser rejecting that specific FedEx email (check whether Cory's inbox row shows the 📦 icon; no icon = parser said not-a-shipping-email). Already-confirmed working today: Tracking Inbox loads (429 fixed), tap-to-copy, auto-forward, inbox view.
...
**2026-07-22 follow-up #4 — household dedupe fix (backend `0b2c4ab`, mobile `76c0a83`, OTA `92966b68`):** Cory's first real package (FedEx to his tracking address) didn't appear on Active for anyone despite the email landing in his inbox. Root cause: the mobile `deduplicatePackages` grouped by carrier+month (predates households) and collapsed Cory's FedEx package into Cliff's existing FedEx card. Fixes: (1) mobile dedupes primarily by tracking number (carrier+month only for number-less packages); (2) webhook duplicate-check is now household-wide (`household_id` + tracking number, newest row wins) so two members' emails about the same physical package converge on one row instead of creating twins.
