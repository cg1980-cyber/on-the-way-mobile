...
**Next action when work resumes:** task #53 is **shipped end-to-end (all 4 layers live)** as of 2026-06-14 — needs real-world testing, ideally by inviting Cory so multi-member features (filter chips, recipient assignment, gift mode) are exercised. **Cliff TODOs:** (1) add `BREVO_API_KEY` to Railway so invites email for real (until then the app shows the code/link in-app); (2) commit the mobile repo — `App.js`, `docs/invite.html`, `HOUSEHOLD_SCHEMA_MIGRATION.sql`, `HOUSEHOLD_INVITE_CODE.sql`, `PROJECT_STATE.md` are all uncommitted locally (sandbox doesn't push the mobile repo). **Known follow-ups:** parser smart-routing not yet built — webhook currently defaults a package's recipient to whoever owns the tracking address that received the email (sensible default; name-matching against other members is the future refinement). Task #49 (EasyPost) still paused; deprioritized behind #53.
...
- **Mobile UI DONE, shipped via EAS** (update group `628949be-b39f-498c-af7c-a7c3b4d9211b`, preview branch, 2026-06-14). `App.js`: household screen (Settings → Household — rename, member list w/ owner/pending badges, invite form, "Have an invite code?" join field, leave/remove), per-member filter chips above the active list (shown only with >1 member), "Who it's for" recipient chips + 🎁 gift-mode checkboxes on the detail screen. `docs/invite.html`: static landing page (mirrors `reset.html`) that fetches the invite by token and shows the code + instructions.
...
2. **Task #53 — Pillar 1 (Household accounts) — Full version.** Locked 2026-06-07. Multi-member, shared feed, per-person filtering, gift mode, email invitations, parser smart-routing by shipping-label name matching, member roles (owner / member), departure/handoff logic. ~15–30 focused hours / 2–3 weeks of evenings. The headline MVP differentiator delivered with polish. **Sub-tasks:** schema design (`households` + `household_members` + `household_id` on packages + `recipient_member_id` + `hidden_from`), RLS rewrites in Supabase, backend CRUD + email-invitation endpoints, parser update to match shipping-label names against household members, mobile settings UI for household management, per-member filter chips on active list, gift mode toggle on detail screen, owner/member role logic, departure-handoff (what happens to assigned packages when a member leaves), migration script for Cliff's existing 9 packages, onboarding update for new signups. **Load-bearing first step:** schema + RLS design pass before any code lands. Easy to get wrong, painful to fix once data has flowed through. Smaller tiers (Minimum viable, MVP-aligned) considered and rejected 2026-06-07: Cliff wants the polished version that matches the marketing doc.
