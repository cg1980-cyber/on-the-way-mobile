...
3. **Push notifications — code shipped 2026-07-06, delivery blocked on Firebase.** Full pipeline exists (device registration → `push_tokens` → household-wide Expo push on new package / status change). Needs: `PUSH_TOKENS_SETUP.sql` run + Firebase FCM v1 credentials uploaded to EAS + one rebuild with `google-services.json`. The household-wide delivery alert ("notify whoever's home, not just the buyer") is a differentiator no competitor ships.
